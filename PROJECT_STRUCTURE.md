...
| Offline trace audit generation | `python scripts/generate_offline_trace_audit.py` |
...
18. R062 records the pre-registration and passed candidate-logging smoke gate
    before any formal online repair.
19. R063 records the no-go decision for formal online repair before the current
    submission; any future formal repair should begin as R064+.
20. Manuscript logic is maintained in `PAPER_PLAN.md`.
21. Derived figure/table assets are generated into `figures/`.
22. Figure/table provenance is tracked in `figures/FIGURE_ASSET_INDEX.md`.
...
| R062 | Repair pre-registration and smoke result: records the candidate-logging smoke gate and a tiny passed logging smoke without adding formal empirical results. |
| R063 | Formal repair go/no-go decision: records no-go for formal online repair before the current submission and reserves future formal repair work for R064+ pre-registration/results. |
...
| Trace schema | R031 extracted trace columns and privileged-state row construction; R061 extends it to candidate-state rows. | `foresight_hil/experiments/trace.py` returning stable rows from controller + privileged state. | Done first pass |
...
| Repair pre-registration | R062 records the candidate-logging smoke, required artifacts, and passed smoke result. | `results/r062_repair_preregistration/`. | Done first pass |
| Formal repair decision | R063 records the no-go decision for formal online repair before the current submission. | `results/r063_formal_repair_decision/`. | Done first pass |
